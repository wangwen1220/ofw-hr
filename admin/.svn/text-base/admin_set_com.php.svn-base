...
elseif($act == 'set_meal') {
	...
	$smarty->assign('config_num',$config_num);
...
elseif($act == 'set_meal_add_save')
{
	...
	$setsqlarr['resume_search']=intval($_POST['resume_search']);
...
elseif($act == 'reg_service_save') {
	...
	!$db->query("UPDATE ".table('config')." SET value='$reg_service' WHERE name='reg_service' LIMIT 1")?adminmsg('保存失败', 1):"";
	!$db->query("UPDATE ".table('config')." SET value='$config_num' WHERE name='month_freesetmeal_num' LIMIT 1")?adminmsg('保存失败', 1):"";
