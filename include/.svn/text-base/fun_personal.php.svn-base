...
*/		
	$row['company_url']=url_rewrite('QS_companyshow',array('id0'=>$row['company_uid'],'addtime'=>$row['company_addtime']));
...
*/
	...
	foreach ($id as $v){
		$sql = "SELECT del FROM ".table('company_interview')." WHERE did=$v";
		$res = $db->getone($sql);
		if ($res['del'] == 1){//企业已删除
			$db->query("Delete from ".table('company_interview')." WHERE did=$v");
		}
		elseif ($res['del'] == 0){//还没有任何状态
			$setsqlarr['del']=2;
			$wheresql=" did=$v  AND resume_uid=".intval($uid)." ";
			updatetable(table('company_interview'),$setsqlarr,$wheresql);
		}
	}
	return true;	
...
function get_favorites($offset,$perpage,$get_sql= '')
{
	...
	$select=" f.*,j.jobs_name,j.addtime as jobs_addtime,j.companyname,j.company_addtime,j.company_id,j.wage_cn,j.district_cn,j.deadline,j.refreshtime,j.click,j.education_cn,j.experience_cn";
...
*/		
	//查询是否已申请		
	$res = $db->getone("select did from ".table('personal_jobs_apply')." WHERE jobs_id='{$row['jobs_id']}' LIMIT 1");
	if (!empty($res)){
		$row['if_apply'] = 1;
	}
	else{
...
function del_sms($id,$uid)
{
	...
	$sqlin=implode(",",$id);
	if (!preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin)) return false;
	$sql="Delete from ".table('feed_back')." WHERE did IN (".$sqlin.") ".$uidsql."";
	echo $sql;exit;
	return $db->query($sql);
}
function check_jobs_apply($jobs_id,$resume_id,$p_uid)//查询七天内是否投递
{
	global $db,$timestamp;
	$sql = "select did,apply_addtime from ".table('personal_jobs_apply')." WHERE personal_uid = '".intval($p_uid)."' AND jobs_id='".intval($jobs_id)."'  AND resume_id='".intval($resume_id)."' ORDER BY apply_addtime DESC  LIMIT 1";
	$res = $db->getone($sql);
	if ($res){
		if ( ($timestamp-$res['apply_addtime']) <= 3600*24*365){//七天内投递,一年内
			if(  (date('z',$timestamp)-date('z',$res['apply_addtime'])) <7  ){
				return true;
			}
			else{
				return false;
...
*/
		//查询此条申请是否有面试通知 resume_uid
		$res = $db->getone("select did from ".table('company_interview')." WHERE jobs_id='{$row['jobs_id']}' AND resume_uid='{$row['personal_uid']}' LIMIT 1");
		if (!empty($res)){
			$row['if_interview'] = 1;
		}
		else{
...
//	if (!$db->query("Delete from ".table('personal_jobs_apply')." WHERE did IN (".$sqlin.") ".$uidsql."")) return false;
	...
	foreach ($del_id as $v){
		$sql = "SELECT del FROM ".table('personal_jobs_apply')." WHERE did=$v";
		$res = $db->getone($sql);
		if ($res['del'] == 1){//企业已删除
			$db->query("Delete from ".table('personal_jobs_apply')." WHERE did=$v");
		}
		elseif ($res['del'] == 0){//还没有任何状态
			$setsqlarr['del']=2;
			$wheresql=" did=$v  AND personal_uid=".intval($uid)." ";
			updatetable(table('personal_jobs_apply'),$setsqlarr,$wheresql);
		}
	}
	return true;	
}
...
function count_interview($uid,$look=NULL)
{
	global $db;
	$uid=intval($uid);
	$wheresql=" WHERE  resume_uid='{$uid}' AND del=0";
...
function count_personal_jobs_apply($uid,$look=NULL)//第二个参数:是否被查看
{
	global $db;
	$wheresql=" WHERE personal_uid='{$_SESSION['uid']}' AND del=0";
