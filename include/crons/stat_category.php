...
global $_CFG,$timestamp;
	...
	$option = " AND (audit=1 OR audit=4)  AND display=1 AND ($timestamp <= deadline OR deadline=0 ) AND $timestamp <= setmeal_deadline AND (company_audit=1 OR company_audit=4) ";
	//岗位分类
	...
	while($row = $db->fetch_array($result))
	{
		//循环子分类,主分类ID为$row['id']
		$pid = $row['id'];
		$sql = "SELECT * FROM ".table('category_jobs')." WHERE parentid=$pid";
		$res = $db->query($sql);
		while ($sub_row = $db->fetch_array($res)){
			//二级分类,各分类的职位数
			$in_jobwheresql=" WHERE `category` REGEXP ',{$sub_row['id']},' ";//匹配二级分类
			$jobtotal=$db->get_total("SELECT COUNT(*) AS num FROM ".table('jobs').$in_jobwheresql.$option);		
			$jobtotal=$jobtotal>0?"({$jobtotal})":'';
			//二级分类,各分类所投的简历数
#			$in_resumewheresql=" WHERE subclass='{$sub_row['id']}'";
#			$resumetotal=$db->get_total("SELECT COUNT(DISTINCT pid ) AS num FROM ".table('resume_jobs')." {$in_resumewheresql} ");
			...
			$sql = "UPDATE ".table('category_jobs')." SET stat_jobs='{$jobtotal}',stat_resume='{$resumetotal}'  WHERE id='{$sub_row['id']}' LIMIT 1";
			$db->query($sql);			
		}

		//加上二级分类的数据
		$in_jobwheresql=" WHERE `subclass` REGEXP ',$pid,' ";//匹配二级分类
		$subcount = $db->get_total("SELECT COUNT(*) AS num FROM ".table('jobs').$in_jobwheresql.$option);
		$jobtotal=$subcount>0?"(".$subcount.")":'';
		//1级分类,各分类所投的简历数
